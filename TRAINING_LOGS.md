...
## 3D CNN Model (Original Implementation)
...
# MobileNet-LSTM Model (Current Implementation)

The MobileNet-LSTM model represents an architectural improvement over the 3D CNN model, utilizing transfer learning from MobileNetV2 (pretrained on ImageNet) combined with LSTM layers for temporal modeling. Training logs for this model will be added after successful training runs.

Key differences from 3D CNN model:
- Uses MobileNetV2 as feature extractor
- Bidirectional LSTM for temporal patterns
- Attention mechanism for focusing on important frames
- Transfer learning from ImageNet weights

Training logs will be recorded here after running `train_mobilenet_LSTM.py`.
```
